 ...
 * 
 * @note The value of tau should be carefully chosen based on the problem requirements and constraints.
 *
...
#define OLDEST(x, y) (frames[x].age < frames[y].age)
	int virtual_time = curr_proc->ktime + curr_proc->utime;
	...
	for (i = 0; i < NR_FRAMES; i++)
	{
		...
		if (frames[i].owner == curr_proc->pid)
		{
			...
			{
				if (virtual_time - frames[i].age > tau)
				{
